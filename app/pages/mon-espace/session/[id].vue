...
}>(() => `/api/mon-espace/session/${route.params.id}`)
...
async function quitter() {
  await zoom.quitter()
  await navigateTo(privee.value ? '/mon-espace/coaching-prive' : '/mon-espace/sessions')
}
...
const retour = computed(() => (privee.value ? '/mon-espace/coaching-prive' : data.value?.modulesCouverts[0] ? `/mon-espace/module/${data.value.modulesCouverts[0].slug}` : '/mon-espace/sessions'))
</script>
  ...
  <div v-if="data" class="sur-sombre flex min-h-screen flex-col bg-encre text-white">
    <header class="flex flex-wrap items-center justify-between gap-3 border-b border-encre-800 px-4 py-3 lg:px-6">
      ...
      <div class="flex items-center gap-3">
        ...
        <NuxtLink :to="retour" class="hidden text-[13.5px] text-[#b9b4c4] hover:text-white lg:inline">← Retour au module</NuxtLink>
      </div>
    </header>
