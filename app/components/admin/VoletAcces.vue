...
<template>
  <div v-if="data">
    <div class="flex flex-wrap items-start justify-between gap-4">
      <div>
        <h2 class="font-title text-[22px] font-light">Administration des accès — rôles de la plateforme</h2>
        <p class="mt-2 max-w-[720px] text-[13.5px] text-discret">
          Trois rôles distincts. Les comptes admin se paramètrent section par section (écran 07) ;
          les comptes formateurs ouvrent le dashboard formateur (D), avec ou sans le volet coaching
          privé.
        </p>
      </div>
    ...
    <!-- Les trois rôles de l'écran 07b -->
    <div class="mt-5 grid gap-3 sm:grid-cols-3">
      <section class="rounded-[12px] border border-ligne-douce bg-white p-4">
        ...
        <p class="mt-1.5 text-[12.5px] text-texte">
          Dashboard admin complet, sections cochées une à une à la création du compte (écran 07).
          Le référencement se sépare en deux droits : « contenu » pour les Titles, meta
          descriptions et partages, « réglages avancés » pour les slugs publiés, l’indexation et
          les canonicals. Double vérification à la connexion.
        </p>
      </section>
      <section class="rounded-[12px] border border-ligne-douce bg-white p-4">
        <p class="text-[14px] font-bold text-encre">Formateur simple</p>
        <p class="mt-1.5 text-[12.5px] text-texte">
          Dashboard formateur : vue d’ensemble, profil public, modules & inscriptions, sessions de
          coaching, profils des apprenants concernés, revenus. Pas de coaching privé.
        </p>
      </section>
      <section class="rounded-[12px] border border-ligne-douce bg-white p-4">
        <p class="text-[14px] font-bold text-encre">Formateur avec coaching privé</p>
        <p class="mt-1.5 text-[12.5px] text-texte">
          Tout l’accès Formateur simple + section Coaching privé : demandes le concernant, sujets
          soumis par les apprenants, séances planifiées, revenus coaching (50 000 FCFA/h). Profil
          proposé au coaching sur /formateurs.
        </p>
      </section>
    </div>
    ...
    >
      <h3 class="font-title text-[19px] font-light">Créer un compte administrateur</h3>
    ...
    >
      <tr v-for="compte in data.comptes" :key="compte.id">
        ...
        <td class="px-4 py-3 text-[13px]" :class="compte.sections === 'toutes' && 'text-discret'">
          {{ perimetre(compte) }}
        </td>
        <td class="px-4 py-3 text-right whitespace-nowrap">
          <span v-if="compte.sections === 'toutes' && !compte.revocable" class="text-discret">—</span>
          <button
      ...
      <tr v-for="f in data.formateurs" :key="f.id">
        ...
        <td class="px-4 py-3 text-right whitespace-nowrap">
          <NuxtLink to="/admin/formateurs" class="text-[12.5px] underline">Modifier</NuxtLink>
          <button
          ...
          <button
            v-else
            class="ml-3 text-[12.5px] text-social underline"
            @click="activation = f"
          >
    ...
    <div v-if="edition" class="mt-6 rounded-[14px] border border-social bg-white p-6">
      <h3 class="font-title text-[19px] font-light">Droits du compte</h3>
    ...
    <div v-if="revocation" class="mt-6 rounded-[14px] border border-erreur bg-[#fdeeee] p-6">
      <h3 class="font-title text-[19px] font-light text-erreur-fonce">
